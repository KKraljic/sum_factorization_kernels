...
template<int dim, int degree, typename Number>
void adjust_total_number_of_cells_to_block_sizes(unsigned int in_n_cells[], unsigned int out_n_cells[],
                                                 EvaluationDGLaplacian<dim, degree, Number> &evaluator,
                                                 const unsigned int n_cells_tot) {
    ...
    out_n_cells[0] =
            std::max(n_cells_tot / (out_n_cells[1] * out_n_cells[2]) / VectorizedArray<Number>::n_array_elements, 1U) *
            VectorizedArray<Number>::n_array_elements;
}
...
void run(const unsigned int n_tests) {
    ...
    unsigned long long vector_size = (unsigned long long) pow(2, 34) - 1;
    EvaluationDGLaplacian<dim, degree, Number> evaluator;
    ...
    sprintf(region_tag, "Inner-Loop-n-%u", n_cells_tot);
