...
void run(const unsigned int n_tests) {
        ...
        unsigned long long vector_size = 100000000;
        ...
        adjust_total_number_of_cells_to_block_sizes<dim, degree, Number>(initial_n_cells, adjusted_n_cells, evaluator, n_cells_tot);
        ...
        for (unsigned int ib = 0; ib < evaluator.n_blocks[2]; ++ib)
            for (unsigned int jb = 0; jb < evaluator.n_blocks[1]; ++jb)
                for (unsigned int kb = 0; kb < evaluator.n_blocks[0]; ++kb)
                    for (unsigned int i = ib * evaluator.blz; i < std::min(evaluator.n_cells[2], (ib + 1) * evaluator.blz); ++i)
                        for (unsigned int j = jb * evaluator.bly; j < std::min(evaluator.n_cells[1], (jb + 1) * evaluator.bly); ++j) {
                            evaluator.do_inner_loop
                                    (kb * evaluator.blx, std::min(evaluator.n_cells[0], (kb + 1) * evaluator.blx), j, i,
